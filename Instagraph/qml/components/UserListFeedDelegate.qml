...
Column {
    ...
    Flow {
        visible: like_count != 0 ? true : false
        width: parent.width
        anchors.horizontalCenter: parent.horizontalCenter
        spacing: units.gu(1)
    ...
    Column {
        ...
        Text {
            visible: caption && caption.text ? true : false
            text: caption && caption.text ? Helper.formatUser(caption.user.username) + ' ' + Helper.formatString(caption.text) : ""
            wrapMode: Text.WordWrap
            width: parent.width
            textFormat: Text.RichText
            onLinkActivated: {
        ...
        Repeater {
            model: userPhotosCommentsModel

            Text {
                visible: c_image_id == pk && comment.text ? true : false
                text: c_image_id == pk && comment.text ? Helper.formatUser(comment.user.username) + ' ' + Helper.formatString(comment.text) : ""
                wrapMode: Text.WordWrap
                width: parent.width
                textFormat: Text.RichText
                onLinkActivated: {
